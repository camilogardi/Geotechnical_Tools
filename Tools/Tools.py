...
rectangular and circular surface loads using Boussinesq's methodology.
...
def calc_circular_surcharge(
    ...
    """
    Calculate vertical stress (sigma_z) under a circular uniformly loaded area
    using Boussinesq's solution at a specific point.
    
    For a uniformly loaded circular area, Boussinesq's solution gives:
    - When r = 0 (directly under center):
        sigma_z = q * [1 - (z / sqrt(z^2 + R^2))^3]
    - When r > 0 (offset from center):
        Uses numerical integration of Boussinesq point load solution
    
    This implementation uses analytical solution for circular load.
    
    Args:
        q: Uniform load intensity in kPa (must be >= 0)
        radius: Radius of circular loaded area in meters (must be > 0)
        x_center: X coordinate of calculation point (meters)
        y_center: Y coordinate of calculation point (meters)
        z_values: Array of depth values in meters (must be > 0)
    
    Returns:
        Tuple containing:
        - z_values: np.ndarray with depth values in meters
        - sigma_z: np.ndarray with vertical stress in kPa at each depth
    
    Raises:
        ValueError: If input parameters are invalid
    
    Example:
        >>> z = np.linspace(0.1, 30, 100)
        >>> z_vals, sigma = calc_circular_surcharge(q=100, radius=5, x_center=0, y_center=0, z_values=z)
        >>> print(f"Stress at 10m depth: {sigma[np.argmin(np.abs(z_vals - 10))]:.2f} kPa")
    """
    # Input validation
    if q < 0:
        raise ValueError(f"Load q must be non-negative, got {q}")
    ...
    if np.any(z_values <= 0):
        raise ValueError(f"All depth values must be positive")
    
    # Calculate radial distance from center of loaded area
    r = np.sqrt(x_center**2 + y_center**2)
    
    # Initialize stress array
    sigma_z = np.zeros_like(z_values)
    
    # Calculate stress for each depth
    for i, z in enumerate(z_values):
        if r == 0:
            # On axis (r=0): analytical solution
            # sigma_z = q * [1 - (z^3) / (z^2 + R^2)^(3/2)]
            term = z**3 / (z**2 + radius**2)**(3/2)
            sigma_z[i] = q * (1 - term)
        else:
            # Off axis (r>0): use simplified approximation
            # Discretize circular load into ring elements and integrate
            n_rings = 20  # Number of ring elements for integration
            ring_radii = np.linspace(0, radius, n_rings + 1)
            
            stress_sum = 0.0
            for j in range(n_rings):
                r_inner = ring_radii[j]
                r_outer = ring_radii[j + 1]
                r_ring = (r_inner + r_outer) / 2  # Center of ring
                
                # Area of ring element
                area_ring = np.pi * (r_outer**2 - r_inner**2)
                
                # Point load from this ring element
                dP = q * area_ring
                
                # Distance from ring to calculation point (approximate)
                # For each ring, integrate around circumference
                n_theta = 12  # Points around circumference
                for k in range(n_theta):
                    theta = 2 * np.pi * k / n_theta
                    x_load = r_ring * np.cos(theta)
                    y_load = r_ring * np.sin(theta)
                    
                    # Distance from load point to calculation point
                    dx = x_center - x_load
                    dy = y_center - y_load
                    R = np.sqrt(dx**2 + dy**2 + z**2)
                    
                    # Boussinesq point load solution (divide by n_theta for averaging)
                    if R > 1e-10:
                        stress_sum += (3 * (dP / n_theta) * z**3) / (2 * np.pi * R**5)
            
            sigma_z[i] = stress_sum
    
    return z_values, sigma_z
